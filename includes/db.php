...
Class Misc {
    ...
    public static function error_handler($num, $err, $file, $line) {
        ...
        $headers .= 'From: Administradora Anelu <noreply@administradoranelu.com>' . "\r\n";
        $html = "";
...
//            echo "<code style='border:1px solid black; background-color:white!important; border-radius:5px;'>" . $html . "</code>";
    ...
    /**
     * agrega un espacio a la izquierda
     * @param String $cadena
     * @return String 
     */
    public static function spaceIzq($string) {
        return ' '.$string;
    }

    /**
     * da formato a un strin a cuenta bancaria
     * @param String $cadena
     * @return String 
     */
    public static function formatAccount($account) {
        return substr($account, 0 , 4).'-'.substr($account, 4 , 4).'-'.substr($account, 8,2).'-'.substr($account,10,10);
    }
    /**
...
Class ACTION {
    const CONSULTA_ESTADO_DE_CUENTA      = 1;
    const CONSULTA_AVISO_DE_COBRO        = 2;
    const CONSULTA_DATOS_PERSONALES      = 3;
    const CONSULTA_JUNTA_CONDOMINIO      = 4;
    const CONSULTA_MENSAJES_RECIBIDOS    = 5;
    const CONSULTA_EDO_CTA_INMUEBLE      = 6;
    const CAMBIO_DE_CLAVE                = 7;
    const INICIO_REGISTRO_PAGO           = 8;
    const COMPLETAR_REGISTRO_PAGO        = 9;
    const AUTORIZAR_PRERECIBO            = 10;
    const VER_CARTELERA                  = 11;
    const VER_CANCELACION_GASTOS         = 12;
    const CONSULTA_HISTORICO_OPERACIONES = 13;
    const ACTUALIZA_DATOS_PERSONALES     = 14;
    const CONSULTA_SOPORTES_FACTURACION  = 15;
    const CONSULTA_MOVIMIENTO_FONDO      = 16;
    const CONSULTA_RECIBOS_CANCELADOS    = 17;
}